...
namespace SoftwareTechnology.Models.DAO
{
    public class ModifySubjectProgram
    {
        ...
        public List<ListSubject> GetList(int branchID)
        {
            ...
            if (dt.Month == 7 || dt.Month == 8 || dt.Month == 9 || dt.Month == 10 || dt.Month == 11)
            {
                List<ListSubject> list = new SoftwareTechnologyDBContext().SubjectPrograms.Where(x => x.BranchID == branchID && (x.SemesterID == 3 || x.SemesterID == 5 || x.SemesterID == 7)).Select(x => new ListSubject { name = x.Subject.Name, credit = x.Subject.Credit, subjectID = x.Subject.ID, semesterID = x.SemesterID }).ToList();
                return list;
            }
            else
            {
                List<ListSubject> list = new SoftwareTechnologyDBContext().SubjectPrograms.Where(x => x.BranchID == branchID && (x.SemesterID == 4 || x.SemesterID == 6 || x.SemesterID == 8)).Select(x => new ListSubject { name = x.Subject.Name, credit = x.Subject.Credit, subjectID = x.Subject.ID, semesterID = x.SemesterID }).ToList();
                return list;
            }
        }
    }
